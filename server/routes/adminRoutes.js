...
router.use(['/clients', '/clients/:id', '/data/:id', '/import-csv', '/visibility/:clientId', '/access-history'], sensitiveLimiter);
...
router.get('/clients', async (_req, res) => {
  ...
  const enriched = (data || []).map(client => ({
    ...client,
    preferences: getClientPreference(client.client_id)
  }));
  res.json(enriched);
});
...
router.post('/import-csv', async (req, res) => {
  ...
  const importMode = data[0]?.import_mode || 'daily';
  const hasAggregatedData = importMode === 'aggregated';
  ...
  res.json({
    ...
    warning: hasAggregatedData
      ? 'Arquivo agregado por período/campanha importado usando a data final do relatório em cada linha.'
      : null,
  });
});
